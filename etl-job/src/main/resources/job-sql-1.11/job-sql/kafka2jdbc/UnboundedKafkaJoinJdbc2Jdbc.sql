...
insert into gmv
select max(log_ts),
 item, COUNT(*) as order_cnt, max(currency_time), cast(sum(amount_kg) * max(rate) as DOUBLE)  as gmv,
 max(precise_timestamp), max(precise_time),  max(gdp)
 from (
 select cast(o.ts as VARCHAR) as log_ts, o.item as item, o.order_id as order_id, c.currency_timestamp as currency_time,
